...
def comp_data(art_stunting, art_wasting, model_stunting, model_wasting, age_start, age_end):
    art_s_mean = art_stunting.query('age_start >= {} and age_end <= {}'.format(age_start, age_end)).mean().mean()
    ms_mean = model_stunting.mean()
    art_s_mean = trans(art_s_mean, ROUND_TO)
    ms_mean = trans(ms_mean, ROUND_TO)
    print(f'Artifact stunting age mean = {art_s_mean} versus Model age mean {ms_mean}')
    art_s_median = art_stunting.query('age_start >= {} and age_end <= {}'.format(age_start, age_end)).median().median()
    ms_median = model_stunting.median()
    art_s_median = trans(art_s_median, ROUND_TO)
    ms_median = trans(ms_median, ROUND_TO)
    print(f'Artifact stunting age medial = {art_s_median} versus Model age median {ms_median}')

    aw_mean = art_wasting.query('age_start >= {} and age_end <= {}'.format(age_start, age_end)).mean().mean()
    mw_mean = model_wasting.mean()
    aw_mean = trans(aw_mean, ROUND_TO)
    mw_mean = trans(mw_mean, ROUND_TO)
    print(f'Artifact wasting age mean = {aw_mean} versus Model age mean {mw_mean}')
    aw_median = art_wasting.query('age_start >= {} and age_end <= {}'.format(age_start, age_end)).median().median()
    mw_median = model_wasting.median()
    aw_median = trans(aw_median, ROUND_TO)
    mw_median = trans(mw_median, ROUND_TO)
    print(f'Artifact wasting age mean = {aw_median} versus Model age mean {mw_median}')
...
def test_cgf(art_stunting, art_wasting):
    ...
    for param in [(TIME_FIRST, AGE_FIRST), (TIME_SECOND, AGE_SECOND)]:
        ...
        #pop = sim.get_population().query('age >= {} and age <= {}'.format(param[1][0], param[1][1]))
